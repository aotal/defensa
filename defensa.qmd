...
def embed_youtube(div_id, video_id, start, end, width=500, height=280, autoplay=1, mute=1, playback_speed=1.5):
    ...
    if (typeof YT === 'undefined' || typeof YT.Player === 'undefined') {{
        ...
        window.onYouTubeIframeAPIReady = function() {{
            createPlayer("{div_id}", "{video_id}", {start}, {end}, {playback_speed});
        }};
    }} else {{
        createPlayer("{div_id}", "{video_id}", {start}, {end}, {playback_speed});
    }}

    function createPlayer(div_id, video_id, start, end, playback_speed) {{
            ...
            events: {{
                ...
                'onStateChange': function(event) {{
                    if (event.data == YT.PlayerState.PLAYING) {{
                        checkTime(event.target, start, end);
                    }}
                }}
            }}
        }});
    ...
    function checkTime(player, start, end) {{
        if (player.getCurrentTime() >= end) {{
            player.seekTo(start);
        }} else {{
            setTimeout(function() {{ checkTime(player, start, end); }}, 1000);
        }}
    }}
    </script>
    """
...
::: notes
Recordar que hablaremos de braquiterapia excepto en el artículo 3 que pasaremos por encima en el caso de sumar las dosis de externa y BT
:::
...
::: notes
Hablar de que el ICRU89 es el documento heredero de las recomendaciones de GEC-ESTRO. 
...
::: notes
Solamente imagen en 3D. 

Aunque los US son una modalidad prometedora no es la estándar.
:::
...
::: notes
Hablar también del tiempo de exploración.
...
::: notes
Hablar de la disponibilidad. 

Modalidad de imagen para la zona pélvica.
:::
...
::: notes
Se aprecia el cérvix, en el CT no. 

Aumento del volumen segmentado del CTV en CT.

Dificultad en la BT adaptativa con CT.
:::
...
::: notes
Volver a comentar la solución del registro vs reconstrucción directa. 

Hay que recordar que la adquisición de imágenes no se hace en el mismo lugar ni al mismo tiempo.

Aplicadores de titanio, apantallamiento del canal.
:::
...
::: notes
Colocamos las posiciones de la fuente sobre la imagen. 

Manualmente o por otros métodos.
:::
...
-   El objetivo es encontrar el conjunto discreto de puntos de parada de la fuente sobre la secuencia de imágenes.

-   Por otro lado, hemos de hacer corresponder univocamente dichas coordenadas a un parámetro de longitud.

```{python}
embed_youtube("player2", "hXDZ8aUN9-A", start=30, end=52, width=500, height=280, autoplay=1, mute=1, playback_speed=2)
```
...
-   Se identifican directamente los puntos de parada sobre la imagen.
...
::: notes
Decir que al formar la trayectoria de la fuente pueden afectar los artefactos de movimiento
:::
...
::: notes
Hechas de materiales de Z alto Son muy habituales y están codificadas para ver el primer punto de parada. 

Dificultad de cuando se cruzan dummies de varios canales y el artefacto a ser materiales de Z alto.
:::
...
::: notes
Hechas de materiales con "hidrógenos"(Por la resonancia) Al ser líquidos, problemas de contrucción de dummies de canal. 

De ahí la dificultad para las agujas.

Límite constructivo por el diámetro de la fuente
:::
...
::: notes
Colocamos el Template mediante las bolas. 

En CT se ve el template perfectamente, no en el caso de la MRI.
:::
...
::: notes
Pasamos de querer un conjunto de puntos a una transformación lineal que nos ligue el origen de la MRI/CT y la posición del aplicador
:::
...
::: notes
Varian lo tiene pero no he trabajado con el sistema. 

Aunque seguro que está diseñado de una manera parecida.
:::
...
```
::: notes
Explicar muy por encima el algebra de transformaciones. 

Recordar que se han desarrollado mucho los cálculos algebraicos sobre sistemas informáticos. AI.
:::
...
Las transiciones las transformaciones. 

Los nodos las piezas. Intentemos ponernos en el lugar del diseñador de elementos. 

Él tomará en origen de la pieza donde le convenga
:::
...
::: notes
Hacer notar que la configuración de un aplicador puede hacer que lo de aplicador rígido no sea tan importante si conseguimos identificar la configuración del aplicador
:::
...
::: notes
Partimos de modelos sin agujas y se las queremos añadir. 

No estaba presente la idea de modificar el aplicador. Ponernos en que estamos en 2017
:::
...
::: {.column width="50%"}
![](img/Utrecht_tool1.png){fig-align="center" width="100%"}

![](img/Utrecht_tool2.png){fig-align="center" width="100%"}
:::
:::
...
Utilizamos el concepto del free-length
:::
...
::: notes
Recordar que no había modelo del TB
:::
...
::: notes
Recordar que no se ve el template de agujas en MRI
:::
...
::: notes
Intentar encontrar técnicas para encontrar la configuración óptima del TB para el implante
:::
...
::: notes
Un buen comienzo es cargar todas las agujas y ver qué posiciones se activan para cubrir el riesgo intermedio poniendo las agujas a la máxima profundidad
:::
...
::: notes
Una vez hecho configurador y generador del aplicador
:::
...
::: notes
Especificidad de la planificación exclusiva para el cérvix y bla bla bla
:::
...
::: notes
Reconocer que los expertos eran la mayoría usuarios de Oncentra y solamente 1 de Varian
:::
...
::: notes
EM solamente de momento para QA

Recordar que la reconstrucción de agujas son trabajos hechos para templates de próstata
:::
...
Es un CT pero como prueba de concepto era suficiente
:::
...
::: notes
Convertir lo no rígido en rígido.
:::
...
Utiliza las dummies de los canales para configurar el aplicador. Se puede volver a el modelo de Utrecht de las diapositivas anteriores
:::
...
#| layout-ncol: 2
#| fig-cap: 
#|   - "[https://aotal.github.io/defensa/](https://aotal.github.io/defensa/)"
#|   - "[https://aotal.github.io/memoria_tesis/](https://aotal.github.io/memoria_tesis/)"
...
# La URL que deseas convertir en un código QR
url = 'https://aotal.github.io/defensa/'
...
# Mostrar la imagen con matplotlib sin ejes
fig, ax = plt.subplots()
...
# La URL que deseas convertir en un código QR
url = 'https://aotal.github.io/memoria_tesis/'
